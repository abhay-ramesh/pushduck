...
const router = upload.createRouter({
  imageUpload: upload.image()
    .onComplete(async ({ storagePath }) => {
      ...
      await storage.upload.file(optimized, `optimized/${storagePath}`);
    })
});
```
...
| Notifications | `onComplete` hook | Email/SMS sending |
...
<Tabs items={["Database", "Webhooks", "Notifications", "Queue", "Analytics"]}>
  <Tab value="Database">
    ...
    const router = upload.createRouter({
      fileUpload: upload.file()
        .onComplete(async ({ file, storagePath, publicUrl, metadata }) => {
  ...
  <Tab value="Webhooks">
    ...
    const router = upload.createRouter({
      fileUpload: upload.file()
        .onComplete(async ({ file, publicUrl }) => {
  ...
  <Tab value="Queue">
    ...
    const router = upload.createRouter({
      fileUpload: upload.file()
        .onComplete(async ({ file, storagePath }) => {
  ...
  fileUpload: upload.file()
    ...
    .onComplete(async ({ file, publicUrl, storagePath, metadata }) => {
      ...
      await yourWebhooks.trigger({
        event: 'file.uploaded',
        data: { url: publicUrl, fileName: file.name }
      });
