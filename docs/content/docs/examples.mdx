...
<Tabs items={["Basic Usage", "With Metadata", "Advanced Configuration", "Error Handling"]}>
        ...
        <div>
          ...
          {files.map(file => (
            <div key={file.id}>
              ...
              {file.publicUrl && <a href={file.publicUrl}>View</a>}
            </div>
          ))}
        </div>
      );
  ...
  <Tab value="With Metadata">
    ...
    export function MetadataUpload() {
      ...
      const [tags, setTags] = useState<string[]>(['summer']);
      ...
      return (
        <div>
          <select value={albumId} onChange={(e) => setAlbumId(e.target.value)}>
            ...
            <option value="work">Work Events</option>
          </select>
    ...
    const uploadRouter = upload.createRouter({
      imageUpload: upload
        ...
        .onComplete(async ({ file, publicUrl, metadata }) => {
          ...
          await saveToDatabase({ url: publicUrl, metadata });
        }),
    });
          ...
          {files.map(file => (
            <div key={file.id} className={`file-item ${file.status}`}>
              ...
              {file.status === "success" && file.publicUrl && (
                <a href={file.publicUrl} target="_blank">View File</a>
              )}
            </div>
          ))}
        </div>
...
<div className="not-prose">
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
    <a href="/docs/quick-start" className="block p-4 border border-fd-border rounded-lg hover:border-fd-border-hover transition-colors">
    ...
    <a href="/docs/providers" className="block p-4 border border-fd-border rounded-lg hover:border-fd-border-hover transition-colors">
      <div className="font-semibold text-fd-foreground">Providers</div>
