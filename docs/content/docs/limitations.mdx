...
## Frontend bindings cover the major frameworks, not all of them

pushduck ships bindings for React, React Native, Vue, Svelte, and Solid. All of
them wrap the same upload engine, so progress, speed, ETA, cancellation, and
metadata behave identically — only the return shape differs, to match each
framework's idiom.

There is **no dedicated binding** for:

- Angular
- Qwik
- Preact (outside React compat)
- Lit / Web Components
- Alpine, htmx, and similar

**What you can still do:**

`pushduck/core` is the upload engine with no framework attached. It exposes
`subscribe(listener)` and `getSnapshot()` — the same contract React's
`useSyncExternalStore`, Svelte stores, and Zustand all speak — so wiring it into
any reactive system is roughly fifty lines. The [Qwik](/docs/integrations/qwik)
and [Fresh](/docs/integrations/fresh) guides are worked examples, and it is
equally usable from plain JavaScript with no framework at all.

You lose nothing functionally by using `pushduck/core` directly; a binding is
purely ergonomic sugar over it.
