...
sequenceDiagram
    ...
    Server->>Server: onComplete(file, publicUrl, metadata)
...
graph TB
    ...
    subgraph Server [Server Layer - pushduck/server]
        ...
        Schema[Schemas<br/>upload.image, upload.file]
...
graph LR
    ...
    Build --> S3Instance[upload object<br/>Schema builders]
    Build --> ConfigObj[config object<br/>Immutable]

    S3Instance --> CreateRouter[upload.createRouter<br/>Define routes]
...
<Mermaid diagram="
graph TB
    Server1[Server: Define Router<br/>const router = upload.createRouter] --> Server2[Server: Export Type<br/>export type AppRouter = typeof router]
    ...
    Server2 --> Infer3[Infer: Output Types<br/>publicUrl, storagePath, metadata]
...
graph TB
    ...
    FinalMeta --> Hooks[Lifecycle Hooks<br/>onComplete]
...
graph TB
    ...
    subgraph After [After Upload]
        Complete[onComplete<br/>Post-processing]
