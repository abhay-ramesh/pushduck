...
<Callout type="info">
  **Web Standards Native**: SolidJS Start's API handlers receive `event.request` as a Web Standard `Request` object, making pushduck integration straightforward with zero overhead.
</Callout>
          ...
          {results().map((result, index) => (
            <div key={index} class="p-2 bg-gray-50 rounded">
              ...
              <div class="text-sm">
                ...
                <a href={result.publicUrl} target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline ml-1">
                  {result.publicUrl}
                </a>
              </div>
              {result.presignedUrl && (
