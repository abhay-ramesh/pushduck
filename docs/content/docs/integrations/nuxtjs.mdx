...
<Callout type="info">
  **Web Standards Native**: Nuxt.js server routes use Web Standard `Request`/`Response` objects, making pushduck integration straightforward with zero overhead.
</Callout>
...
Nuxt uses the Vue binding, `pushduck/vue`. It exposes the same `useUploadRoute`
composable as every other pushduck client — the only difference is that state
comes back as Vue refs, so destructuring in `<script setup>` keeps reactivity.

<Callout type="info">
  `pushduck/vue` requires Vue 3.3+. It is an optional peer dependency, so
  nothing extra is installed for projects that do not use it.
</Callout>
...
<template>
  <div class="space-y-4">
    ...
    <div v-for="file in files" :key="file.id" class="flex gap-2 items-center">
      <span>{{ file.name }}</span>
    ...
    <button v-if="files.length" @click="reset">Clear</button>
  </div>
</template>
```
...
useHead({ title: "File Upload Demo" });
</script>
...
Any contextual data your UI holds can travel with the upload and is available in
server middleware, lifecycle hooks, and path generation.

```vue title="components/AlbumUpload.vue"
<script setup lang="ts">
import { ref } from "vue";
import { useUploadRoute } from "pushduck/vue";

const albumId = ref("vacation-2026");
