...
<Steps>
  ...
  <Step>
    **Create Express server with upload routes**
    ...
    // `app.use` rather than `app.all('/api/upload/*')`: the wildcard form
    // throws at startup on Express 5, whose path-to-regexp requires a *named*
    // wildcard. `use` mounts identically on v4 and v5 and matches both
    // `/api/upload` and anything below it.
...
app.use('/api/upload/public', toExpressHandler(uploadRouter.handlers));

// Private upload route (with auth)
app.use('/api/upload/private', authenticateToken, toExpressHandler(uploadRouter.handlers));
...
app.use('/api/upload', uploadLimiter, toExpressHandler(uploadRouter.handlers));
...
// Public uploads
router.use('/public', toExpressHandler(uploadRouter.handlers));

// Private uploads (requires authentication)
router.use('/private', authenticateToken, toExpressHandler(uploadRouter.handlers));
