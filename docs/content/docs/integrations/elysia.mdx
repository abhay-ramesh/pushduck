...
<Callout type="info">
  **Web Standards Native**: Elysia exposes `context.request` as a Web Standard `Request` object, making pushduck integration straightforward with zero overhead.
</Callout>
...
Splitting the methods works too, if you prefer to be explicit or need
different middleware on each:
...
Or use Elysia's `.mount`, which hosts any Web-standard handler:

```typescript title="server.ts"
const app = new Elysia().mount('/api/upload', uploadRouter.handler);
```

<Callout type="info">
  All three are verified against a real Elysia server running under Bun in
  pushduck's test suite — `.all`, the split handlers, and `.mount`.
</Callout>
