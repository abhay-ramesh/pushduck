---
title: Overview
description: Universal file uploads that work with any web framework - from modern Web Standards to legacy APIs
---
...
<Callout type="info">
  **Universal Design**: Pushduck uses Web Standards (Request/Response) at its core, making it compatible with both modern and legacy frameworks without framework-specific code.
</Callout>
...
<Cards>
  ...
  <Card 
    ...
    description="Frameworks with custom request/response objects"
...
<Tabs items={["Modern (Web Standards)", "Next.js", "Express", "Fastify"]}>
  <Tab value="Modern (Web Standards)">
    ...
    app.get('/api/upload/*', (ctx) => {
      return uploadRouter.handlers.GET(ctx.request); // or c.req.raw
    });

    app.post('/api/upload/*', (ctx) => {
      return uploadRouter.handlers.POST(ctx.request);
    });
    ```
  </Tab>
...
<Steps>
  ...
  <Step>
    **Legacy Framework Adapters**
    ...
    For frameworks with custom APIs, simple adapters convert between formats:
