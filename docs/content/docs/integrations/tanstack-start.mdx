...
<Callout type="info">
  **Web Standards Native**: TanStack Start's API handlers receive `{ request }` as a Web Standard `Request` object, making pushduck integration straightforward with zero overhead.
</Callout>
          ...
          {results.map((result, index) => (
            <div key={index} className="p-2 bg-gray-50 rounded">
              ...
              <div className="text-sm">
                ...
                <a href={result.publicUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline ml-1">
                  {result.publicUrl}
                </a>
              </div>
              {result.presignedUrl && (
...
<Callout type="success">
  **Modern React + Pushduck**: TanStack Start's modern React architecture combined with pushduck's universal design creates a type-safe file upload solution.
</Callout> 