...
<Callout type="info">
  **Web Standards Native**: SvelteKit server endpoints use Web Standard `Request`/`Response` objects, making pushduck integration straightforward with zero overhead.
</Callout>
...
SvelteKit uses the Svelte binding, `pushduck/svelte`. `createUploadRoute` returns a
readable store plus the imperative controls, so `$upload` auto-subscribes in
both Svelte 4 and Svelte 5.

<Callout type="info">
  `pushduck/svelte` requires Svelte 4+. It implements the store contract
  structurally and does not import `svelte/store`, so it adds nothing to your
  bundle beyond the upload logic itself.
</Callout>
...
<script lang="ts">
  ...
  const upload = createUploadRoute<AppUploadRouter>("imageUpload", {
    endpoint: "/api/upload",
...
<div class="space-y-4">
  ...
  {#if $upload.files.length}
    <button on:click={upload.reset}>Clear</button>
  {/if}
</div>
```
...
Stores remain first-class under runes, so the same code works unchanged. Derive
from the store where you want a local reactive value:

```svelte title="src/lib/components/RunesUpload.svelte"
<script lang="ts">
  import { createUploadRoute } from "pushduck/svelte";

  const upload = createUploadRoute("imageUpload", { endpoint: "/api/upload" });

  let percent = $derived(Math.round($upload.progress));
  let done = $derived($upload.files.filter((f) => f.status === "success"));
</script>

<p>{percent}% — {done.length} finished</p>
```

### Passing Client Metadata

```svelte title="src/lib/components/AlbumUpload.svelte"
<script lang="ts">
  import { createUploadRoute } from "pushduck/svelte";

  let albumId = "vacation-2026";
  const upload = createUploadRoute("imageUpload", { endpoint: "/api/upload" });
