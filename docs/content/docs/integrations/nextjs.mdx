...
Pushduck provides straightforward integration with both Next.js App Router and Pages Router through universal handlers that work with Next.js's Web Standards-based API.
...
export const uploadRouter = upload.createRouter({
  imageUpload: upload.image().maxSize("5MB"), // [!code highlight]
...
```
  ...
  <Step>
    **Create API route**
        ...
        ```typescript title="app/api/upload/route.ts" tab="App Router"
        ...
        ```typescript title="pages/api/upload/[...path].ts" tab="Pages Router"
        import { uploadRouter } from '@/lib/upload';
...
Next.js App Router uses Web Standards, making integration straightforward:
...
export const { upload, useUpload } = createUploadClient<AppUploadRouter>({
  endpoint: '/api/upload'
});
```
...
<Callout type="success">
  **Next.js Ready**: Pushduck works with both Next.js App Router and Pages Router, providing the same great developer experience across all Next.js versions.
</Callout> 