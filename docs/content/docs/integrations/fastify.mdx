...
<Callout type="info">
  **Custom Request/Response API**: Fastify uses `request`/`reply` objects instead of Web Standards, so pushduck provides the `toFastifyHandler` adapter for straightforward integration.
</Callout>
...
<Cards>
  ...
  <Card title="Production Ready" description="Enterprise features">
