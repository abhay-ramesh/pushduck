...
<Callout type="info">
  **Web Standards Native**: Remix loader and action functions use Web Standard `Request`/`Response` objects, making pushduck integration straightforward with zero overhead.
</Callout>
...
Remix is React, so it uses `pushduck/client` directly.
...
```tsx title="app/lib/upload-client.ts"
