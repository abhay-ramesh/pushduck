...
<Callout type="info">
  **Web Standards Native**: Fresh API routes use Web Standard `Request`/`Response` objects, making pushduck integration straightforward with zero overhead.
</Callout>
      ...
      <Tab value="Deno">
        ...
        ```json title="import_map.json"
        {
          "imports": {
            ...
            "pushduck/core": "https://esm.sh/pushduck@latest/core"
          }
        }
        ```
      </Tab>
  ...
  <Step>
    **Create API route**
    ...
    export const handler: Handlers = {
      async GET(req) {
        return uploadRouter.handler(req);
      },
      async POST(req) {
        return uploadRouter.handler(req);
      },
...
Fresh islands run Preact in the browser. pushduck ships no Preact-specific
binding, and does not need one — `pushduck/core` exposes the upload engine
directly and wires into Preact hooks in a few lines.
...
export default function FileUpload() {
  ...
  const [state, setState] = useState<UploadEngineState | null>(null);
  ...
  useEffect(() => {
    ...
    return engine.subscribe(() => setState(engine.getSnapshot()));
  }, []);
      ...
      {state.files.map((file) => (
        <div key={file.id} class="flex gap-2 items-center">
          ...
          {file.status === "uploading" && (
            <button onClick={() => engineRef.current?.cancel(file.id)}>
              Cancel
            </button>
          )}
        </div>
