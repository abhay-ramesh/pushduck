...
export const { upload, storage } = createUploadConfig()
...
const uploadRouter = upload.createRouter({
  ...
  documentUpload: upload.file().maxSize("50MB").accept(["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"]),
});

export const { GET, POST } = uploadRouter.handlers;

// Export the router type for client-side type safety
export type Router = typeof uploadRouter;
```

<Callout type="info">
  **What's happening here?** - `upload.createRouter()` creates a type-safe upload
  handler - `upload.image()` and `upload.file()` provide validation and TypeScript
      ...
      {uploadedFiles.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {uploadedFiles.map((file) => (
            <div key={file.storagePath} className="space-y-2">
              <img
                src={file.publicUrl}
                alt="Uploaded image"
...
<div className="p-4 border rounded-lg">
    <h3 className="font-semibold mb-2">Other Providers</h3>
  <p className="text-sm text-gray-600 mb-3">
  ...
  <div className="p-4 border rounded-lg">
    ...
    <a
      href="/docs/guides/migrate-to-enhanced-client"
      className="text-blue-600 text-sm hover:underline"
    >
      Migration Guide →
    </a>
...
<Callout type="info">
  **Find Pushduck useful?** Star us on
