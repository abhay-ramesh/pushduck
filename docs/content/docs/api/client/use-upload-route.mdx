    ...
    <div>
      ...
      {files.map((file) => (
        <div key={file.id}>
          ...
          {file.status === "success" && <a href={file.publicUrl}>View</a>} // [!code highlight]
        </div>
      ))}
...
const { uploadFiles } = useUploadRoute<AppRouter>('imageUpload', {
  ...
  onSuccess: (results) => {
    ...
    results.forEach(file => {
      console.log('File URL:', file.publicUrl);          // Permanent URL
...
<TypeTable
  type={{
    ...
    storagePath: {
      description: "Object key/path in storage for database storage and operations",
      type: "string | undefined",
    },
    presignedUrl: {
...
const { uploadFiles, files } = useUploadRoute<AppRouter>('documentUpload', {
  ...
  onSuccess: (results) => {
    ...
    updateDocuments(results.map(file => ({
      url: file.publicUrl,              // Permanent access
    ...
    <div>
      ...
      {files.map(file => (
        <div key={file.id}>
          ...
          {file.status === 'success' && <img src={file.publicUrl} alt={file.name} width={100} />}
        </div>
      ))}
    </div>
  );
...
const { uploadFiles, uploadedFiles } = useUploadRoute<AppRouter>('attachments', {
  onSuccess: (results) => {
    setValue('attachments', results.map(r => r.publicUrl))
  }
})
