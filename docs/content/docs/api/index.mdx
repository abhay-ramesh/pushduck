...
<Cards>
  ...
  >
    - `useUploadRoute` - Core upload hook with progress tracking
...
function UploadComponent() {
  ...
  const handleUpload = async (file: File) => {
    ...
    console.log('Public URL:', uploaded?.url);           // Permanent access
    console.log('Download URL:', uploaded?.presignedUrl); // Temporary access (1 hour)
  };
