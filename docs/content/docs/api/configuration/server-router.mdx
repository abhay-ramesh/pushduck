...
<Steps>
  <Step>
    **Create your upload route**
    ...
    export const { POST, GET } = createS3Handler(s3Router)
    ```
  ...
  <Step>
    **Export router types**
    ...
    const { s3, createS3Handler, storage } = uploadConfig
      .cloudflareR2({
    ...
    export { s3, createS3Handler, storage }
...
```typescript
import { createS3Client } from "pushduck/s3";
