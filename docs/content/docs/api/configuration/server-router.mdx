...
<Steps>
  <Step>
    **Create your upload route**
    ...
    export const { POST, GET } = uploadRouter.handlers
    ```
  ...
  <Step>
    **Export router types**
    ...
    const { upload, storage } = createUploadConfig()
    ...
    // Define your router
    const uploadRouter = upload.createRouter({
      imageUpload: upload.image().maxSize('4MB').maxFiles(10),
      documentUpload: upload.file().maxSize('10MB').accept(['application/pdf']).maxFiles(1)
    })
...
The `upload` schema builders provide a fluent API for defining your upload requirements:
...
<Tabs items={["Images & Documents", "Mixed Media", "Any Files"]} groupId="file-types">
  <Tab value="Images & Documents">
    ```typescript
    const uploadRouter = upload.createRouter({
      mixedUpload: upload.object({
        images: upload.image().maxSize('4MB').maxFiles(5),
        pdfs: upload.file().maxSize('10MB').accept(['application/pdf']).maxFiles(2),
        documents: upload.file().maxSize('5MB').accept(['application/vnd.openxmlformats-officedocument.wordprocessingml.document']).maxFiles(3)
      })
    })
    ```
  </Tab>
...
const uploadRouter = upload.createRouter({
  images: upload.image().paths({ prefix: "images" }),
  documents: upload.file().paths({ prefix: "documents" })
});
```
...
const uploadRouter = primaryConfig.upload.createRouter({
  productImages: primaryConfig.upload.image().maxSize("4MB").maxFiles(10),
...
const uploadRouter = upload.createRouter({
  imageUpload: upload.image()
    .maxSize("4MB")
    .maxFiles(10)
    .onComplete(async ({ file, publicUrl, storagePath, metadata }) => {
...
```typescript
const ecommerceRouter = upload.createRouter({
  productImages: upload.image()
    .maxSize('5MB')
    .maxFiles(8)
    ...
    .onComplete(async ({ files, metadata }) => {
...
```typescript
const docsRouter = upload.createRouter({
  contracts: upload.file()
    .maxSize('25MB')
    .accept(['application/pdf'])
    .maxFiles(1)
    .middleware(async ({ req }) => {
  ...
  proposals: upload.object({
    pdfs: upload.file().maxSize('50MB').accept(['application/pdf']).maxFiles(3),
    documents: upload.file().maxSize('25MB').accept(['application/vnd.openxmlformats-officedocument.wordprocessingml.document']).maxFiles(5),
  }).middleware(async ({ req }) => {
