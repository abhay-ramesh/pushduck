  ...
  <Step>
    **Export router types**
    ...
    const s3Router = s3.createRouter({
      ...
      documentUpload: s3.file().maxFileSize('10MB').accept(['application/pdf']).maxFiles(1)
    })
  ...
  <Tab value="Images & Documents">
    ...
    const s3Router = s3.createRouter({
      mixedUpload: s3.object({
        ...
        pdfs: s3.file().maxFileSize('10MB').accept(['application/pdf']).maxFiles(2),
        documents: s3.file().maxFileSize('5MB').accept(['application/vnd.openxmlformats-officedocument.wordprocessingml.document']).maxFiles(3)
      })
    })
    ```
  </Tab>
...
const docsRouter = s3.createRouter({
  ...
  proposals: s3.object({
    pdfs: s3.file().maxFileSize('50MB').accept(['application/pdf']).maxFiles(3),
    documents: s3.file().maxFileSize('25MB').accept(['application/vnd.openxmlformats-officedocument.wordprocessingml.document']).maxFiles(5),
  }).middleware(async ({ req }) => {
