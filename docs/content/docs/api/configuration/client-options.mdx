...
<Tabs items={["useUploadRoute Hook", "Multiple Endpoints", "Custom Hook"]} groupId="hook-api">
  <Tab value="useUploadRoute Hook">
    ```typescript
    import { useUploadRoute } from 'pushduck/client'
    import type { AppRouter } from '@/lib/upload'

    export function ImageUploadComponent() {
      const { uploadFiles, files, isUploading, error, reset } = useUploadRoute<AppRouter>('imageUpload', {
        onSuccess: (results) => {
