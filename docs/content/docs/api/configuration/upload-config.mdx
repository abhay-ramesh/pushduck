...
const { storage, upload, config } = createUploadConfig()
...
.hooks({
  ...
  onComplete: async ({ file, publicUrl, storagePath, metadata }) => {
    console.log(`Upload completed: ${file.name} -> ${publicUrl}`)
...
file.publicUrl = "https://bucket.s3.region.amazonaws.com/path/file.jpg"

// With custom domain
file.publicUrl = "https://cdn.yourdomain.com/path/file.jpg"
```
...
```typescript
// Client-side usage
const { uploadFiles } = useUpload('fileUpload', {
  onSuccess: (results) => {
    results.forEach(file => {
      ...
      console.log('Storage Path:', file.storagePath);  // Object key/path in storage
    });
  }
});
```
...
- **`publicUrl`** - Permanent URL for public file access, CDN caching, and embedding
- **`presignedUrl`** - Temporary download URL (expires in 1 hour) for secure access
- **`storagePath`** - Object key/path in storage for database storage and storage API operations

Use the appropriate URL based on your access requirements:
- Public files: Use `publicUrl` for permanent access
- Private files: Use `presignedUrl` for time-limited access
- File operations: Use `storagePath` with storage API
...
interface UploadConfig {
  ...
  hooks?: {
    onStart?: (ctx: { file: any; metadata: any }) => Promise<void> | void
    onComplete?: (ctx: {
      file: any
      publicUrl: string    // Permanent file URL
      storagePath: string  // Object key/path in storage
      metadata: any
    }) => Promise<void> | void
    onError?: (ctx: {
      file: any
