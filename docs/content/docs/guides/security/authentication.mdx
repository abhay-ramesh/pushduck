...
```typescript
export const router = createUploadRouter({
  apiKeyUploads: uploadSchema({
    document: { maxSize: "25MB", maxCount: 1 },
  }).middleware(async ({ req }) => {
...
export const router = createUploadRouter({
  ...
  moderatorUploads: uploadSchema({
    image: { maxSize: "10MB", maxCount: 20 },
  }).middleware(async ({ req }) => {
    const { userId, roles } = await authenticateUser(req);
  ...
  userUploads: uploadSchema({
    image: { maxSize: "5MB", maxCount: 5 },
  }).middleware(async ({ req }) => {
