      ...
      <div className="image-preview-grid">
        {files.map((file) => (
          <div key={file.id} className="image-preview">
            {file.status === "success" && (
              <img src={file.presignedUrl || file.publicUrl} alt={file.name} className="preview-image" />
            )}
...
<Callout type="info">
**⚠️ Important:** Pushduck handles **file uploads only**. Image processing (resizing, optimization, format conversion) requires external tools. The examples below show **integration patterns** using `.onComplete()` hooks.
...
const uploadRouter = upload.createRouter({
  optimizedImages: upload.image()
    .maxSize('15MB')
    .maxFiles(10)
    .accept(['image/jpeg', 'image/png', 'image/webp'])
    .dimensions({ maxWidth: 1920, maxHeight: 1080 })
    .onComplete(async ({ file, publicUrl, metadata }) => {
      // ⚠️ This downloads the file from S3 (inbound bandwidth)
      const imageBuffer = await fetch(publicUrl).then(r => r.arrayBuffer());
...
const uploadRouter = upload.createRouter({
  responsiveImages: upload.image()
    ...
    .onComplete(async ({ file, publicUrl, storagePath, metadata }) => {
      ...
      await saveImageVariants(storagePath, variants, metadata.userId);
  }),
});
      ...
      profilePicture: upload.image()
        ...
        .onComplete(async ({ file, publicUrl, storagePath, metadata }) => {
          ...
          await updateUserAvatar(metadata.userId, {
            original: publicUrl,
            small: getVariantUrl(storagePath, 'avatar-small'),
            medium: getVariantUrl(storagePath, 'avatar-medium'),
            large: getVariantUrl(storagePath, 'avatar-large')
          })
      })
    })
    ```
            ...
            <div key={file.id} className="image-item">
              {file.status === "success" && (
                <div className="image-wrapper">
                  <img src={file.presignedUrl || file.publicUrl} alt={file.name} />
                  <button
