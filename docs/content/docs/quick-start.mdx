...
<Callout type="tip">
  **File paths**: By default, files are uploaded with just the filename (e.g., `photo.jpg`). To organize files into folders, see [Path Configuration](/docs/api/configuration/path-configuration).
</Callout>
    ...
    <div>
      ...
      {files.map((file) => (
        <div key={file.id}>
          ...
          {file.status === 'success' && <img src={file.publicUrl} alt={file.name} />}
        </div>
      ))}
    </div>
  );
