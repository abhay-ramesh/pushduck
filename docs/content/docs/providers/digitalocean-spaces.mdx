...
- **S3 Compatible**: Works with S3 tools and libraries
...
export const { upload } = createUploadConfig()
...
const imageUrl = file.publicUrl; // Automatically CDN-accelerated
...
const imageUpload = upload
  ...
  .onComplete(async ({ file, storagePath }) => {
    ...
    await setObjectMetadata(storagePath, {
...
.hooks({
  onComplete: async ({ file, storagePath, metadata }) => {
    ...
    if (metadata.category === 'temp') {
      await scheduleCleanup(storagePath, { days: 7 });
    }
  }
})
```
