...
const uploadRouter = s3.createRouter({
      imageUpload: s3.image().maxFileSize("5MB").accept(["image/jpeg", "image/png", "image/webp"]),
      fileUpload: s3.file().maxFileSize("10MB").accept(["application/pdf", "text/plain", "image/*"]),
});
